define([
	...
	var TreeNode = declare("dijit._TreeNode", [_Widget, _TemplatedMixin, _Container, _Contained, _CssStateMixin], {
		...
		_updateItemClasses: function(item){
			...
			if (item.leaf && this.tree.needCheckbox) {
				domClass.remove(this['checkboxNode'], 'checkboxHide');
			}
	...
	var Tree = declare("dijit.Tree", [_Widget, _KeyNavMixin, _TemplatedMixin, _CssStateMixin], {
		...
		postCreate: function(){
			...
			this.own(
				...
				on(this.containerNode, on.selector(".treeNodeCheckbox", 'click'), function(evt) {
					self._onCheck(registry.getEnclosingWidget(this), evt);
				})
			);
		...
		_setCheckedItemsAttr: function(items) {
			...
			if(_.isEmpty(items)){
				//empty data list
				array.forEach(tree.get('checkedItems'),function(item){
						$(tree.getNodesByItem(item.id)[0].domNode).find('input[type=checkbox]').prop('checked',false);
				});
				tree.checkedItems = [];
			}else{
				array.forEach(items, function(item) {
					...
					if (item.parent) {
						tree._onExpandoClick({node: tree.getNodesByItem(item.parent)[0]});//TBD
					}
					if (tree.needCheckbox) {
						if(!item.id){
							return false;
						}
						var needCheckedNodes = $(tree.getNodesByItem(item.id)[0].domNode);
						if(needCheckedNodes && !_.isEmpty(needCheckedNodes)){
							/*fix bug for using only click to check item*/
							$(tree.getNodesByItem(item.id)[0].domNode).find('input[type=checkbox]').prop('checked',false);
							/*
							$(tree.getNodesByItem(item.id)[0].domNode).find('input[type=checkbox]').click();
							*/
							///* so slow to response to all click events
							$(tree.getNodesByItem(item.id)[0].domNode).find('input[type=checkbox]')[0].checked = true;
							//*/
						}
					} else {
						//tree.getNodesByItem(item.id)[0] && $(tree.getNodesByItem(item.id)[0].domNode).find('.dijitTreeLabel').click();
						/*
						if(	tree.getNodesByItem(item.id)[0]){
							console.log($(tree.getNodesByItem(item.id)[0].domNode).find('.dijitTreeLabel'));
						}
						*/
					}
					tree.emit('Check',item);
		...
		_onBeforeCheck: function(/*TreeNode*/ nodeWidget, /*Event*/ e) {
			this['onBeforeCheck'](nodeWidget.item, nodeWidget, e);
		},

		_onCheck: function(/*TreeNode*/ nodeWidget, /*Event*/ e){
			this['onCheck'](nodeWidget.item, nodeWidget, e);
		},
